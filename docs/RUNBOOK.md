...
2. Find the last known-good production deployment (cross-check its commit
   SHA against what the homepage displayed before the incident — the page
   itself is the record of what was live and when).
3. Use **Instant Rollback** (the `...` menu on that deployment → Promote
   to Production, or `vercel rollback` via the CLI).
4. Confirm: reload the production URL, check the commit SHA on the page
   matches the deployment you rolled back to.
...
`/api/health/db`'s reported migration version and the homepage's commit
SHA are the two observables for telling these apart quickly: if the
commit SHA is what you expect but the app is misbehaving against the
data, suspect the schema.
